            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg w-fit">
                    <button
                        ...
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition ${type === 'mcq' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-600 hover:text-gray-900'}`}
                    >
                        Multiple Choice
                    </button>
                    <button
                        ...
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition ${type === 'true_false' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-600 hover:text-gray-900'}`}
                    >
                        True / False
                    </button>
                </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
                    />
                ...
                <div className="space-y-4">
                    ...
                    {type === 'true_false' ? (
                            ...
                            {options.map((opt, idx) => (
                                <button
                                    ...
                                    className={`flex-1 p-4 rounded-xl border-2 transition-all font-bold text-center ${opt.is_correct
                                        ? 'border-indigo-600 bg-indigo-50 text-indigo-700 drop-shadow-sm'
                        ...
                        <div className="space-y-3">
                            {options.map((opt, idx) => (
                                <div key={opt.tempId} className={`flex items-center gap-3 p-3 border rounded-xl transition ${opt.is_correct ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 bg-white'}`}>
                                    {/* Correct Radio */}
                                    <div className="flex items-center justify-center p-2 cursor-pointer" onClick={() => setCorrectOption(idx)}>
                                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${opt.is_correct ? 'border-indigo-600' : 'border-gray-300'}`}>
                                            {opt.is_correct && <div className="w-2.5 h-2.5 rounded-full bg-indigo-600"></div>}
                                        </div>
                                    </div>
                            ...
                            {options.length < 6 && (
                                <button
                                    type="button"
                                    onClick={addOption}
                                    className="text-sm font-medium text-indigo-600 hover:text-indigo-800 transition flex items-center mt-2 px-2"
                                >
                ...
                <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
                    <button
                        ...
                        className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition shadow-sm"
                    >
